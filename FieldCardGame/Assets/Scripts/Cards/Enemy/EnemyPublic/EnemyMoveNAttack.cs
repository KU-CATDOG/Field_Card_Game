...
public class EnemyMoveNAttack : ICard
{   //actually.. for only minotaurs
    ...
    private RangeType attackType = RangeType.Distance;
    public int _dmg
    {
        get { return dmg; }
        set { dmg = value; }
    ...
    public List<Coordinate> GetAvailableTile(Coordinate pos)
    {
        ...
        return pos.GetDistanceAvailableTile(range, rangeType, true, true); //check isMAX 
    }   
    ...
    public IEnumerator CardRoutine(Character caster, Coordinate center)
    {
        ...
        foreach (Coordinate i in path)
        {
            ...
            {
                if (GameManager.Instance.Map[i.X + dirs[j][0], i.Y + dirs[j][1]].CharacterOnTile is Player)
                {
                    yield return GameManager.Instance.StartCoroutine(caster.HitAttack(GameManager.Instance.Map[center.X, center.Y].CharacterOnTile, dmg));
                }
            }
        }
